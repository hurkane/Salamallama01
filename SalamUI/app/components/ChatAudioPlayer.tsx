...
const ChatAudioPlayer = ({ audioPreview }) => {
  ...
  useEffect(() => {
    if (waveformContainerRef.current) {
      waveSurferRef.current = WaveSurfer.create({
        ...
        minPxPerSec: 40, // Ensure the waveform has minimum width per second of audio
        backend: "MediaElement", // Use the MediaElement backend for better browser compatibility
      });
  ...
  useEffect(() => {
    if (audioPreview && waveSurferRef.current) {
      console.log("Loading audio preview:", audioPreview);
      fetch(audioPreview)
        .then((response) => {
          if (!response.ok) {
        ...
        .then((blob) => {
          ...
          waveSurferRef.current.on("ready", () => {
            console.log("Audio loaded successfully from blob URL:", blobURL);
          });
